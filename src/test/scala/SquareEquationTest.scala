...
class SquareEquationTest extends AnyWordSpecLike with Matchers {
  ...
  "square equation" should {
    ...
    "have one root" in {
      val roots: List[Double] = SquareEquation.solve(a = 1.0, b = -2.0, c = 1.0)
      roots should be (1.0 :: 1.0 :: Nil)
    }

    "throw exception when a = 0" in {
      assertThrows[IllegalArgumentException] {
        SquareEquation.solve(a = 0.0, b = 1.0, c = 1.0)
      }
    }

    "throw exception when a = Inf" in {
      assertThrows[IllegalArgumentException] {
